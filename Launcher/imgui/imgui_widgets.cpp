...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (window->SkipItems & ImGuiItemFlags_ButtonRepeat)
    ...
    it_hovered->second = ImClamp(it_hovered->second + (1.f * ImGui::GetIO().DeltaTime * (hovered || pressed ? 1.f : -1.5f)), 0.0f, 1.0f);
    ...
    it_active->second = ImClamp(it_active->second + (1.f * ImGui::GetIO().DeltaTime * (held ? 1.f : -1.f)), 0.0f, 1.0f);
    it_active->second *= style.Alpha;

    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(23, 25, 24, int(255 * fill)), style.FrameRounding+5);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(45, 45, 45, int(255 * fill)), style.FrameRounding + 5);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), style.FrameRounding +5);

    PushStyleColor(ImGuiCol_Text, ImColor(130, 130, 130, int(255 * fill)).operator ImVec4());
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);

    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(100 * it_hovered->second)).operator ImVec4());
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);

    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * it_active->second)).operator ImVec4());
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, ImVec2(pos.x + 280, pos.y + 40));
    ...
    it_active->second = ImClamp(it_active->second + (2.f * ImGui::GetIO().DeltaTime * (*v ? 1.f : -1.5f)), 0.0f, 1.f);
    it_active->second *= style.Alpha;
    ...
    {
        ImGui::RenderFrame(total_bb.Min, total_bb.Max, ImColor(23, 25, 24), true, 5);
        window->DrawList->AddRect(ImVec2(total_bb.Min.x, total_bb.Min.y), ImVec2(total_bb.Min.x + 280, total_bb.Min.y + 40), ImColor(32, 35, 34), 5, 15, 1.5);

        window->DrawList->AddRectFilled(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 10), ImVec2(total_bb.Min.x + 30, total_bb.Min.y + 30), ImColor(28, 30, 29), 5);
        window->DrawList->AddRect(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 10), ImVec2(total_bb.Min.x + 30, total_bb.Min.y + 30), ImColor(94, 159, 255, int(255 * it_hovered->second )), style.FrameRounding +5, 15, 1.6f);


        if (*v)
        {
          
            RenderCheckMark(window->DrawList, { total_bb.Max.x - 265, total_bb.Min.y + 3 + square_sz / 2 }, ImColor(97, 174, 248, int(255 * it_active->second * 2)), 10.f);
        }
        else
        {
            RenderCheckMark(window->DrawList, { total_bb.Max.x - 265, total_bb.Min.y + 3 + square_sz / 2 }, ImColor(97, 174, 248, int(255 * it_active->second * 2)), 10.f);
        ...
        if (label_size.x > 0.0f)
            window->DrawList->AddText(ImVec2(total_bb.Min.x + 40, total_bb.Min.y + 20 - ImGui::CalcTextSize(label).y / 2), ImColor(150, 153, 153, 200 + int(255 * it_hovered->second)), label);
    }
    ImGui::PopFont();
